...
// Set up the process.threadpoolStats() function
function setup_threadpoolStats() {
  const _threadpoolStats = process.threadpoolStats;

  const tpValues = new Float64Array(6);

  process.threadpoolStats = function threadpoolStats() {
    _threadpoolStats(tpValues);
    return {
      submitted: tpValues[0],
      started: tpValues[1],
      done: tpValues[2],
      queued_max: tpValues[3],
      idle_max: tpValues[4],
      idle_min: tpValues[5]
    };
  };
